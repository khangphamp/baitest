...
function StepTwo({phrase18, setStep}) {
  const [onCopy, setOnCopy] = useState(false);

  const HanldeGetValueName = useMemo(() => phrase18.map(i => i.name).join(' '))
  return (
    <div className="flex flex-col h-screen">
            ...
            <div className="w-full h-[170px] bg-white py-8 flex flex-col justify-between p-5 shadow-type1">
                ...
                <Button onClick={() => setStep(3)} className="text-black-dark font-san700 text-[17px]">Next Phrase 6 </Button>
            </div>
